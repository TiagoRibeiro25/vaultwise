...
export default function TransactionsClient() {
  ...
  const confirmDelete = async () => {
    ...
    await deleteTransaction(undefined, `/api/transactions/${transactionToDelete}`)
  ...
  return (
    <div className="space-y-6">
      <div className="sm:flex sm:items-center sm:justify-between">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 sm:mt-0 sm:ml-16 sm:flex-none">
          <select
            value={selectedMonth}
            onChange={e =>
              setSelectedMonth(e.target.value === "all" ? "all" : Number(e.target.value))
            }
            className="cursor-pointer rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-700 shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
          >
          ...
          <ExportCsvButton filteredTransactions={filteredTransactions || null} />
          <button
            type="button"
          ...
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
            <thead className="bg-slate-50 dark:bg-slate-950/50">
              <tr>
                <th
                  scope="col"
                  className="py-3.5 pr-3 pl-4 text-left text-sm font-semibold text-slate-900 sm:pl-6 dark:text-slate-200"
                >
                  {t("date")}
                </th>
                <th
            ...
            <tbody className="divide-y divide-slate-200 bg-white dark:divide-slate-800 dark:bg-slate-900">
              {isLoading ? (
                  ...
                  >
                    <div className="flex flex-col items-center justify-center">
                      <div className="flex flex-col items-center justify-center py-12 text-center text-slate-500 dark:text-slate-400">
                        ...
                        <p className="mt-1 text-xs">{t("getStarted")}</p>
                      </div>
                    </div>
                  </td>
                </tr>
                ...
                filteredTransactions?.map(transaction => (
                  ...
                  >
                    <td className="py-4 pr-3 pl-4 text-sm whitespace-nowrap text-slate-500 sm:pl-6 dark:text-slate-400">
                      {format(new Date(transaction.date), "MMM dd, yyyy", {
                        locale: dateLocale,
                      })}
                    </td>
                    <td className="px-3 py-4 text-sm font-medium whitespace-nowrap text-slate-900 dark:text-slate-200">
                    ...
                    <td className="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-6">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="cursor-pointer p-1 text-slate-400 transition-colors hover:text-indigo-600 dark:text-slate-500 dark:hover:text-indigo-400"
                          title={t("edit")}
                        >
                        ...
                        <button
                          ...
                          className="cursor-pointer p-1 text-slate-400 transition-colors hover:text-red-600 dark:text-slate-500 dark:hover:text-red-400"
                          title={t("delete")}
                        >

...
import { ArrowDownRight, ArrowUpRight, Calendar, Search, Wallet } from "lucide-react"
...
export default function HistoryClient() {
  ...
  const { availableYears, availableMonths } = useMemo(() => {
    ...
    return {
      ...
      availableMonths: Array.from(monthsByYear[selectedYear] || []).sort((a, b) => a - b),
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white p-2 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <Calendar className="ml-2 h-5 w-5 text-slate-400" />
          <select
            value={selectedMonth}
            onChange={e => setSelectedMonth(Number(e.target.value))}
            className="cursor-pointer border-none bg-transparent py-1 pr-8 pl-2 text-sm font-medium text-slate-700 focus:ring-0 focus:outline-none dark:text-slate-200"
          >
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="border-b border-slate-200 px-6 py-4 dark:border-slate-800">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
            {t("transactionsFor", {
              month: format(new Date(2000, selectedMonth - 1), "MMMM", {
                locale: dateLocale,
              }),
              year: selectedYear,
            })}
          ...
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
            <thead className="bg-slate-50 dark:bg-slate-950/50">
              <tr>
                ...
                <th
                  scope="col"
                  className="px-3 py-3.5 text-right text-sm font-semibold text-slate-900 sm:pr-6 dark:text-slate-200"
                >
                  {t("amount")}
                </th>
              </tr>
              ...
              {isLoading ? (
                ...
                filteredTransactions.map(transaction => (
                  ...
                  >
                    ...
                    <td className="px-3 py-4 text-right text-sm font-medium whitespace-nowrap sm:pr-6">
                      <div

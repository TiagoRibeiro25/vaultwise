...
export default function BudgetsClient() {
  ...
  const { data: transactions, execute: fetchTransactions } = useApi<Transaction[]>({
  ...
  const budgetsWithSpent: BudgetWithSpent[] = (budgets || []).map(budget => {
    const spent = (transactions || [])
      .filter(t => {
        ...
        return (
          tDate.getMonth() + 1 === budget.month && tDate.getFullYear() === budget.year
        )
      })
  ...
  const confirmDelete = async () => {
    ...
    const result = (await deleteBudgetApi(null, `/api/budgets/${budgetToDelete}`)) as {
      error?: string
    } | null
    if (!result?.error) {
      fetchBudgets()
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none dark:hover:bg-indigo-500"
        >
        ...
        <div className="p-6">
          {loadingBudgets ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{
                          backgroundColor: budget.category?.color || "#cbd5e1",
                        }}
                      >
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                          {formatCurrency(budget.spent)} {t("of")}{" "}
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className={`font-semibold ${getTextColor(budget.percentage)}`}>
                          {budget.percentage.toFixed(0)}%
                        </p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                          {Number(budget.amount) - budget.spent > 0
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="p-1 text-slate-400 transition-colors hover:text-indigo-600 dark:hover:text-indigo-400"
                          title={t("edit")}
                        >
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <button
                onClick={() => handleOpenModal()}
                className="mt-6 inline-flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none dark:hover:bg-indigo-500"
              >
                <Plus className="mr-2 h-4 w-4" />
                {t("createFirst")}
              </button>
